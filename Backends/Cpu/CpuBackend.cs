...
namespace Llama.Backends.Cpu;

public unsafe class CpuBackend : IBackend
{
    public void Dispose()
    {
        // Do nothing
    ...
    public string GetDescription()
    {
        foreach (var line in File.ReadLines("/proc/cpuinfo"))
        {
            ...
            {
                // Format is usually: "model name : AMD Ryzen 7 7800X3D 8-Core Processor"
    ...
    public virtual T* Allocate<T>(ulong size) where T : unmanaged
    {
        return (T*)NativeMemory.AllocZeroed((nuint)size);
    }

    public virtual void Free(void* p)
    {
        NativeMemory.Free(p);
    }

    public virtual void MatMul(float* xout, float* x, float* w, int n, int d)
    {
